...
class ConnectQlik:
    ...
    # ADDED BY RILEY
    def import_appcontent(self, appId, fileName):
        """
        Imports an application
        :param name: Name of application to import
        :param filename: Path and file of qvf
        :returns: HTTP Status Code
        """
        path = 'qrs/appcontent/{0}/uploadfile?externalpath={1}&overwrite=true'.format(appId, fileName)
        headers["Content-Type"] = "application/vnd.qlik.sense.app"
        headers["Connection"] = "Keep-Alive"
        with open(fileName, 'rb') as image:
            return self.post(path, image)        
